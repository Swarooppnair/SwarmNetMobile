...
import com.bitchat.android.features.network.NetworkStatusSheet
...
fun ChatScreen(viewModel: ChatViewModel) {
    ...
    var showNetworkStatusSheet by remember { mutableStateOf(false) }
    ...
    Box(
        ...
        val headerHeight = 96.dp  // Increased to fit 2-row header (nickname row + CHATS title)
    ...
    ChatInputSection(
        ...
        ChatFloatingHeader(
            ...
            onNetworkStatusClick = { showNetworkStatusSheet = true }
        )
    ...
    ChatDialogs(
        ...
        showNetworkStatusSheet = showNetworkStatusSheet,
        onNetworkStatusSheetDismiss = { showNetworkStatusSheet = false },
    )
}
...
private fun ChatFloatingHeader(
    ...
    Surface(
        ...
        TopAppBar(
            ...
            modifier = Modifier.height(headerHeight) // Ensure header fits both rows
        )
    }
}
...
private fun ChatDialogs(
    ...
    showNetworkStatusSheet: Boolean = false,
    onNetworkStatusSheetDismiss: () -> Unit = {},
) {
    val privateChatSheetPeer by viewModel.privateChatSheetPeer.collectAsStateWithLifecycle()
    ...
    if (showNetworkStatusSheet) {
        NetworkStatusSheet(viewModel = viewModel, onDismiss = onNetworkStatusSheetDismiss)
    }
