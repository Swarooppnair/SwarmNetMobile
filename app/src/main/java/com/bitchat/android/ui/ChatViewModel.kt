    ...
    fun triggerSOS(sosTypeLabel: String? = null, sosTypeIcon: String? = null) {
        viewModelScope.launch {
            try {
                ...
                val currentGeohash = (state.selectedLocationChannel.value as? com.bitchat.android.geohash.ChannelID.Location)
                    ?.channel?.geohash ?: ""

                // Try to get actual coordinates via location provider
                val locationProvider = com.bitchat.android.geohash.SystemLocationProvider(getApplication())
                locationProvider.getLastKnownLocation { location ->
                    val lat = location?.latitude ?: 0.0
                    val lon = location?.longitude ?: 0.0
                    val geohash = if (currentGeohash.isNotEmpty()) currentGeohash else {
                        if (lat != 0.0 || lon != 0.0) {
                            try { com.bitchat.android.geohash.Geohash.encode(lat, lon, 6) } catch (_: Exception) { "" }
                        } else ""
                    }
