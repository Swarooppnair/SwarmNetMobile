    ...
    override fun didReceiveMessage(message: BitchatMessage) {
        coroutineScope.launch {
            ...
            if (message.isPrivate) {
                ...
                // Parse incoming SOS messages and show sender's location on the map
                // Format: "🚨 SOS [#geohash] — @nickname needs help"
                if (message.content.startsWith("🚨 SOS")) {
                    try {
                        val geohashMatch = Regex("\\[#([a-z0-9]+)\\]").find(message.content)
                        val geohash = geohashMatch?.groupValues?.getOrNull(1) ?: ""
                        val lat: Double
                        val lon: Double
                        if (geohash.isNotEmpty()) {
                            val coords = com.bitchat.android.geohash.Geohash.decodeToCenter(geohash)
                            lat = coords.first
                            lon = coords.second
                        } else {
                            lat = 0.0; lon = 0.0
                        }
                        com.bitchat.android.features.sos.SosAlertStore.addAlert(
                            com.bitchat.android.features.sos.SosAlert(
                                id = message.id,
